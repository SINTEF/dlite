...
class FortranFormat(Formatter):
    ...
    def func(self, fcn, fmt):
        ...
        if fmt.find("func_arg") == 0:
            ...
            for arg in fcn.args:
                item = self.arg(
                    arg, "decl_arg", length=1, dimension="*", intent="in", bind_c=True
                )
    ...
    def arg(self, arg, fmt="", **kwargs):
        ...
        if fmt == "func_arg":
            ...
            return self.decl_arg.format(
                ftype=ftype, comma="," if attr else "", attr=",".join(attr), name=name
            )
