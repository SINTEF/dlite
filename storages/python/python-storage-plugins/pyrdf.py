...
class pyrdf(dlite.DLiteStorageBase):
    ...
    def from_bytes(cls, buffer, id=None):
        ...
        Returns:
            New instance.
        """
        return from_rdf(data=buffer, id=id, format=cls.options.get("format"))
    ...
    def to_bytes(cls, inst):
        ...
        return to_rdf(
            inst,
            format=cls.options.get("format"),
            base_uri=cls.options.get("base_uri"),
            base_prefix=cls.options.get("base_prefix"),
        )
