...
def _is_valid_uri(value):
    ...
    return bool(re.match(r"^[a-zA-Z+-]+://[a-zA-Z][a-zA-Z0-9_.+/#?,;=-]*$", value))
...
def _value(graph, subject=None, predicate=None, object=None, **kwargs):
    ...
    value = graph.value(subject=subject, predicate=predicate, object=object, **kwargs)
    if not value:
        raise ValueError(
...
def to_graph(inst, graph=None, base_uri="", base_prefix=None, include_meta=None):
    ...
    if inst.is_data:
        ...
        for k in inst.properties:
            ...
            graph.add(
                (prop, DM.hasValue, Literal(inst.get_property_as_string(k, flags=1)))
            )
    else:
        graph.add(
            (
                this,
                RDF.type,
                DM.Entity if inst.meta.uri == dlite.ENTITY_SCHEMA else DM.Metadata,
            )
        )
        if inst.description:
            graph.add((this, DM.hasDescription, Literal(inst.description, lang="en")))
        for d in inst.properties["dimensions"]:
            dim = URIRef(this + sep + d.name)
            graph.add((this, DM.hasDimension, dim))
            graph.add((dim, RDF.type, DM.Dimension))
            graph.add((dim, DM.hasDescription, Literal(d.description, lang="en")))
...
def to_rdf(
    ...
    **kwargs,
    ...
    """
    graph = to_graph(
        inst, base_uri=base_uri, base_prefix=base_prefix, include_meta=include_meta
    )
...
def from_graph(graph, id=None):
    ...
    if id is None:
        ...
        try:
            ...
            raise ValueError("id must be given when graph has move than one entity")
    ...
    if not uuid:
        ...
        if v:
            ...
            uuid = id
            dlite_id = (
                rdfid.split("#", 1)[-1] if "#" in rdfid else rdfid.rsplit("/", 1)[-1]
            )
            rdfid = URIRef(rdfid)
    if uuid:
        uuid = str(uuid)
    ...
    if meta.is_metameta:
        ...
        dims = {
            str(_value(graph, dim, DM.hasLabel)): int(_value(graph, dim, DM.hasValue))
            for dim in dimensions
        }
