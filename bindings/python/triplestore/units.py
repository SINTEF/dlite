...
def pint_registry_lines_from_qudt():
    ...
    for s, p, o in ts.triples([None, QUDT.hasDimensionVector, None]):
        ...
        pint_definition = pint_definition_string(
            parse_qudt_dimension_vector(dimension_vector)
        )
    ...
    for URIb, definition in pint_definitions.items():
        ...
        if unit_name in base_unit_dimensions.keys():
            pint_definition_line = f"{unit_name} = [{base_unit_dimensions[unit_name]}]"
        else:
...
class PintIdentifiers:
    ...
    def is_valid_identifier(self, identifier: str, URI: str, label_name: str) -> bool:
        ...
        return (
            self.URIs[identifier_index] == URI
            and self.label_names[identifier_index] == label_name
        )
