...
MU_TEST(test_tgen_get_substitution)
{
  ...
  sub = tgen_get_substitution(subs, countof(subs), "n", -1);
  mu_check(sub);
  mu_assert_string_eq("n", sub->var);

  sub = tgen_get_substitution(subs, countof(subs), "name", -1);
  mu_check(sub);
  mu_assert_string_eq("name", sub->var);

  sub = tgen_get_substitution(subs, countof(subs), "name", 0);
  mu_check(!sub);
  ...
  sub = tgen_get_substitution(subs, countof(subs), "name", 2);
  mu_check(!sub);

  sub = tgen_get_substitution(subs, countof(subs), "x", -1);
  mu_check(!sub);
}
...
MU_TEST(test_tgen)
{
  ...
  str = tgen("{name} got n={n}!", subs, countof(subs), NULL);
  mu_assert_string_eq("Adam got n=42!", str);
  free(str);

  str = tgen("simple template", subs, countof(subs), NULL);
  mu_assert_string_eq("simple template", str);
  free(str);

  str = tgen("{xname} got n={n}!", subs, countof(subs), NULL);
  mu_check(!str);

  str = tgen("{name:new template} got n={n}!", subs, countof(subs), NULL);
  mu_assert_string_eq("Adam got n=42!", str);
  free(str);

  str = tgen("{name:invalid {{n}} got n={n}!", subs, countof(subs), NULL);
  mu_check(!str);

  str = tgen("invalid } template", subs, countof(subs), NULL);
  mu_check(!str);

  str = tgen("{name:valid {n} } got n={n}!", subs, countof(subs), NULL);
  mu_assert_string_eq("Adam got n=42!", str);
  free(str);
  ...
  str = tgen("{name:valid {n} {n}{} got n={n}!", subs, countof(subs), NULL);
  mu_assert_string_eq("Adam got n=42!", str);
  free(str);

  str = tgen("should {{ work }}!", subs, countof(subs), NULL);
  mu_assert_string_eq("should { work }!", str);
  free(str);

  str = tgen("pi is {pi:templ string{}...", subs, countof(subs), NULL);
  mu_assert_string_eq("pi is 3.14...", str);
  free(str);

  str = tgen("pi is {pi:templ {n{}...", subs, countof(subs), NULL);
  mu_assert_string_eq("pi is 3.14...", str);
  free(str);

  str = tgen("func subst {f:YY}", subs, countof(subs), NULL);
  mu_assert_string_eq("func subst YY", str);
  free(str);

  str = tgen("func subst {f:pi={pi}{}", subs, countof(subs), NULL);
  mu_assert_string_eq("func subst pi=3.14", str);
  free(str);

  str = tgen("func subst: {f}", subs, countof(subs), NULL);
  mu_check(!str);

  str = tgen("func subst {f2:pi={pi} }", subs, countof(subs), NULL);
  mu_assert_string_eq("func subst pi=3.14 ", str);
  free(str);

  str = tgen("func subst {f2}", subs, countof(subs), NULL);
  mu_assert_string_eq("func subst XX", str);
  free(str);

  str = tgen("show loop:\n{loop:  i={i} - data={data}\n}",
             subs, countof(subs), NULL);
...

  {
    char template[] =
      ...
      "and the loop:\n"
      "{loop:  i={i} - data={data}\n}";
    str = tgen(template, subs, countof(subs), NULL);
...
MU_TEST_SUITE(test_suite)
{
  ...
  MU_RUN_TEST(test_tgen_get_substitution);
