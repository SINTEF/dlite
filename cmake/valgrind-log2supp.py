...
def main():
    ...
    parser.add_argument(
        'infile', type=argparse.FileType('rt'),
    ...
    parser.add_argument(
        '--tail', '-t', type=argparse.FileType('rt'),
        help="A file who's content will appended to the output.")
    parser.add_argument(
        '--skip-leak-kinds', '-s', action='store_true',
        help='Whether to skip matching leak kinds.')
    parser.add_argument(
        '--keep-obj', '-k', action='store_true',
        help=('Whether to keep `obj:<library>` in output.  The default is '
              'to replace it with `obj:*`.'))
    parser.add_argument(
        '--exclude-regex', '-x', metavar='REGEX',
